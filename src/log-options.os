...
Процедура ОбработатьНастройкуСпособаВывода(Знач Ключ, Знач Значение)
	...
	Если ПустаяСтрока(Ключ) Тогда
		...
		ОписаниеАппендера.Вставить("Уровень", УровниЛога.Информация);
	...
	Иначе
		...
		Если НастройкаАппендера = Неопределено Тогда
			ВызватьИсключение СтрШаблон("Неверная структура файла. Класс способа вывода {%1} должен быть описан ранее строки {%2}",
				...
				ИмяАппендера + "." + Ключ);
		КонецЕсли;
		Если НРег(Ключ) = НРег(ИМЯ_КЛЮЧА_УРОВЕНЬ()) Тогда
			НовыйУровень = КартаУровней().Получить(Значение);
			Если НовыйУровень = Неопределено Тогда
				ВызватьИсключение "Не нашли уровень по ключу " + Значение;
			КонецЕсли;
			
			НастройкаАппендера.Вставить("Уровень", НовыйУровень);
		Иначе
...
Функция КартаУровней()
	Если Не ЗначениеЗаполнено(мКартаУровней) Тогда
		...
		мКартаУровней = Новый ФиксированноеСоответствие(КартаУровней);

	КонецЕсли;
	
	Возврат мКартаУровней;
КонецФункции
