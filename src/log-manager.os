...
Процедура ОбновитьНастройки() Экспорт
	...
	ОтладочныйКонфигИзСреды = ПолучитьПеременнуюСреды("LOGOS_LEVEL");
	КонфигИзСреды = Неопределено;
	Если ЗначениеЗаполнено(ОтладочныйКонфигИзСреды) И СтрСравнить(ОтладочныйКонфигИзСреды, "DEBUG") = 0 Тогда
		КонфигИзСреды = "logger.rootLogger=DEBUG";
	Иначе
		КонфигИзСреды = ПолучитьПеременнуюСреды("LOGOS_CONFIG");
	КонецЕсли;
	Если ЗначениеЗаполнено(КонфигИзСреды) Тогда
		сообщить("КонфигИзСреды " + КонфигИзСреды);
