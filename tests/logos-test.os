...
Процедура Тест_ДолженПрочитатьНастройкиЛогированияИзФайла() Экспорт
	...
	ЗаписьТекста.Записать("logger.errlog=ERROR"); // простая установка уровня
	ЗаписьТекста.Записать("logger.debuglog=DEBUG, debugfile"); // уровень + аппендер
	ЗаписьТекста.Записать("appender.debugfile=ВыводЛогаВФайл"); // класс аппендера
	ЗаписьТекста.Записать("appender.debugfile.file=/tmp/logostestdebug"); // свойство аппендера
	ЗаписьТекста.Записать("appender.debugfile.anotherprop=hello world"); // свойство аппендера
	...
	Ожидаем.Что(НастройкиЛогаОтладки.СпособыВывода[0].Ключ).Равно("debugfile");
	СвойстваАппендера = НастройкиЛогаОтладки.СпособыВывода[0].Значение;
	Ожидаем.Что(СвойстваАппендера.Получить("file")).Равно("/tmp/logostestdebug");
	Ожидаем.Что(СвойстваАппендера.Получить("anotherprop")).Равно("hello world");
