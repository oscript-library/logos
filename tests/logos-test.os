...
Процедура Тест_ДолженПроверитьВыводДвухЛоговВОДинФайл() Экспорт

	// Дано 

	ФайлЛога = ВременныеФайлы.НовоеИмяФайла("log");

	ЗначениеПеременнойСреды = СтрШаблон("logger.log1=DEBUG, dbgfile;logger.log2=DEBUG, dbgfile;appender.dbgfile=ВыводЛогаВФайл;appender.dbgfile.level=DEBUG;appender.dbgfile.file=%1",
										ФайлЛога);
	...
	// Когда
	Лог1 = Логирование.ПолучитьЛог("log1");
	Лог1.Отладка("Сообщение1");

	Лог2 = Логирование.ПолучитьЛог("log2");
	Лог2.Отладка("Сообщение2");

	Файл = Новый Файл(ФайлЛога);

	// Тогда
	Ожидаем.Что(Файл.Существует(), "Файл с логами создан").Равно(Истина);
	...
	Ожидаем.Что(СтрокиЛога.Количество(), "количество залоггированных сообщений").Равно(2);
	Ожидаем.Что(СтрокиЛога[0], "Первая строка лога").Равно("ОТЛАДКА - [log1] - Сообщение1");
	Ожидаем.Что(СтрокиЛога[1], "Вторая строка лога").Равно("ОТЛАДКА - [log2] - Сообщение2");
	
КонецПроцедуры

Процедура Тест_ДолженПроверитьПриемственностьИерархииЛогов() Экспорт

	// Дано 

	ФайлЛога = ВременныеФайлы.НовоеИмяФайла("log");

	ЗначениеПеременнойСреды = СтрШаблон("logger.mainlog=DEBUG, dbgfilehierarch;appender.dbgfilehierarch=ВыводЛогаВФайл;appender.dbgfilehierarch.level=DEBUG;appender.dbgfilehierarch.file=%1",
										ФайлЛога);
	...
	// Когда
	Лог1 = Логирование.ПолучитьЛог("mainlog.sublog1");
	Лог1.Отладка("Сообщение1");

	Лог2 = Логирование.ПолучитьЛог("mainlog.sublog2");
	Лог2.Отладка("Сообщение2");

	Лог3 = Логирование.ПолучитьЛог("mainlog.sublog1.subsublog");
	Лог3.Отладка("Сообщение3");
	...
	Ожидаем.Что(СтрокиЛога.Количество(), "количество залоггированных сообщений").Равно(3);
	Ожидаем.Что(СтрокиЛога[0], "Первая строка лога").Равно("ОТЛАДКА - [mainlog.sublog1] - Сообщение1");
	Ожидаем.Что(СтрокиЛога[1], "Вторая строка лога").Равно("ОТЛАДКА - [mainlog.sublog2] - Сообщение2");
	Ожидаем.Что(СтрокиЛога[2], "Третья строка лога").Равно("ОТЛАДКА - [m.sublog1.subsublog] - Сообщение3");
...
Процедура Тест_ДолженПроверитьПередачуНастроекАппендеровВИерархииЛогов() Экспорт
	...
	ЧтениеТекста = Новый ЧтениеТекста(ФайлЛогаОшибки);
